...
function Checkbox({
  ...
  return (
    <CheckboxPrimitive.Root
      data-slot="checkbox"
      className={cn(
        "peer border-input dark:bg-input/30", // Removed data-[state=checked] styles here
        ...
        "data-[state=checked]:bg-red-500", // Added red background when checked
        "data-[state=checked]:border-red-500", // Optional: Change border to red too
        className
      )}
      {...props}
    >
      <CheckboxPrimitive.Indicator
        ...
        className={cn(
          ...
          "data-[state=checked]:text-white" // Added white color for the check icon
        )}
      >
        <CheckIcon className="size-3.5" />
      </CheckboxPrimitive.Indicator>
